...
class SampleDatasetGenerator:
    ...
    def generate(self):
        ...
        self.coef = np.sort(np.random.randint(100, 1000, self.n_informative) / 100 * [(-1) ** i for i in range(self.n_informative)])[::-1]
        coef = self.coef

        for n in range(self.n_features - self.n_informative):
            x = np.random.normal(self.mean, self.std, self.n_samples) * self.independence
